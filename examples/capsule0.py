...
S.dem.collectNodes()
S.dtSafety=.8
S.engines=woo.utils.defaultEngines(damping=.4,dynDtPeriod=10)+[BoxFactory(box=((-1,-1,1+i*2),(1,1,3+i*2)),stepPeriod=100,maxMass=3e3,maxNum=-1,massFlowRate=0,maxAttempts=100,attemptPar=50,atMaxAttempts=BoxFactory.maxAttWarn,generator=generators[i],materials=[mat]) for i in range(len(generators))]
