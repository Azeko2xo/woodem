...
O.dem.par.append(pack.gtsSurface2Facets(surf))

sp=pack.SpherePack()
sp.makeCloud(Vector3(-1,-9,30),Vector3(1,-13,32),.2,rRelFuzz=.3)
O.dem.par.append([utils.sphere(c,r) for c,r in sp])

O.scene.engines=utils.defaultEngines(gravity=(0,0,-9.81))+[VtkExport(stepPeriod=100,what=VtkExport.spheres|VtkExport.mesh,out='/tmp/p1-')]
