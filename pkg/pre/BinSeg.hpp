...
struct BinSeg: public Preprocessor {
	...
	WOO_CLASS_BASE_DOC_ATTRS_CTOR(BinSeg,Preprocessor,"Preprocessor for the bin segregation simulation.",
		...
		((vector<Vector2r>,psd,vector<Vector2r>({Vector2r(0.015,.0),Vector2r(.03,1.)}),AttrTrait<>().startGroup("Feed").triggerPostLoad().multiUnit().lenUnit().prefUnit("mm").fractionUnit().prefUnit("%"),"Particle size distribution of generated particles: first value is diameter, second value is cummulative fraction"))
		...
		((vector<Particle::id_t>,holeIds,,AttrTrait<>().noGui(),"Remember ids of hole particles so that they can be removed in the simulation later"))
