...
void TraceGlRep::render(const shared_ptr<Node>& n, GLViewInfo*){
	...
	glBegin(GL_LINE_STRIP);
		for(size_t i=0; i<pts.size(); i++){
			...
			if(!isnan(pts[ix][0])){
				glColor3v(Tracer::lineColor->color(i*1./pts.size()));
				glVertex3v(pts[ix]);
			}
		}
	glEnd();
...
void Tracer::run(){
	...
	for(const auto& p: *dem.particles){
		size_t i=p->id;
		// delete from all
		if(reset){ for(const auto& n: p->shape->nodes) n->rep.reset(); }
		if(modulo[0]>0 && (i+modulo[1])%modulo[0]!=0) continue;
		if(rRange.maxCoeff()>0
