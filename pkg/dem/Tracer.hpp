...
struct Tracer: public PeriodicEngine{
	...
	virtual void run();
	enum{SCALAR_NONE=0,SCALAR_TIME,SCALAR_VEL,SCALAR_ACCEL,SCALAR_RADIUS};
	WOO_CLASS_BASE_DOC_STATICATTRS_PY(Tracer,PeriodicEngine,"Save trace of node's movement",
		...
		((int,scalar,SCALAR_NONE,AttrTrait<>().choice({{SCALAR_NONE,"none"},{SCALAR_TIME,"time"},{SCALAR_VEL,"|vel|"},{SCALAR_ACCEL,"|accel|"},{SCALAR_RADIUS,"radius"}}),"Scalars associated with history points (determine line color)"))
		...
		, /*py*/
			...
			_classObj.attr("scalarAccel")=(int)Tracer::SCALAR_ACCEL;
