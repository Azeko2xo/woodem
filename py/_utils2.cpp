...
BOOST_PYTHON_MODULE(_utils2){
	...
	py::def("importSTL",DemFuncs::importSTL,(py::arg("filename"),py::arg("mat"),py::arg("mask")=0,py::arg("color")=-.999,py::arg("scale")=1.,py::arg("shift")=Vector3r::Zero().eval(),py::arg("ori")=Quaternionr::Identity(),py::arg("threshold")=-1e-6),"Return list of :obj:`particles <woo.dem.Particle>` (with the :obj:`Facet <woo.dem.Facet>` :obj:`shape <woo.dem.Particle.shape>`) imported from given STL file.\nBoth ASCII and binary formats are supported; COLOR and MATERIAL values in the binary format (if given) translate to the :obj:`woo.dem.Shape.color` scalar − that means that color differences are preserved, but not the color as such.\n*scale*, *shift* and *ori* are applied in this order before :obj:`nodal <woo.core.Node>` coordinates are computed. The *threshold* value serves for merging incident vertices: if positive, it is distance in the STL space (before scaling); if negative, it is relative to the max bounding box size of the entire mesh.");
}
