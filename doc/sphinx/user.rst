...
Yade integrates with the the `GNU Triangulated Surface library <http://gts.sourceforge.net>`_, exposed in python via the 3rd party :ref:`external:gts` module. GTS provides variety of functions for surface manipulation (coarsening, tesselation, simplification, import), to be found in its documentation.

GTS surfaces are geometrical objects, which can be inserted into simulation as set of particles whose :ref:`Body.shape` is of type :ref:`Facet` -- single triangulation elements. :ref:`pack.gtsSurface2Facets` can be used to convert GTS surface triangulation into list of :ref:`bodies<Body>` ready to be inserted into simulation via ``O.bodies.append``.

Facet particles are created by default as non-:ref:`Body.dynamic` (they have zero inertial mass). That means that  they are fixed in space and will not move if subject to forces. You can however

* prescribe arbitrary movement to facets using a :ref:`PartialEngine` (such as :ref:`TranslationEngine` or :ref:`RotationEngine`);
* assign explicitly :ref:`mass<State.mass>` and :ref:`inertia<State.inertia>` to that particle;
* make that particle part of a clump and assign :ref:`mass<State.mass>` and :ref:`inertia<State.inertia>` of the clump itself (described below).

.. note::
	Facets can only (currently) interact with :ref:`spheres<Sphere>`, not with other facets, even if they are *dynamic*. Collision of 2 :ref:`facets<Facet>` will not create interaction, therefore no forces on facets.

Import
-------

Yade currently offers 3 formats for importing triangulated surfaces from external files, in the :ref:`yade.ymport` module:
...
The :ref:`external:gts` module provides convenient way of creating surface by vertices, edges and triangles.

Frequently, though, the surface can be conveniently described as surface between polylines in space. For instance, cylinder is surface between two polygons (closed polylines). The :ref:`yade.pack.sweptPolylines2gtsSurface` offers the functionality of connecting several polylines with triangulation.

.. note::
	The implementation of :ref:`yade.pack.sweptPolylines2gtsSurface` is rather simplistic: all polylines must be of the same length, and they are connected with triangles between points following their indices within each polyline (not by distance). On the other hand, points can be co-incident, if the ``threshold`` parameter is positive: degenerate triangles with vertices closer that ``threshold`` are automatically eliminated.
...
There are 2 methods for representing exact volume of the solid in question in Yade: boundary representation and constructive solid geometry. Despite their fundamental differences, they are abstracted in Yade in the :ref:`Predicate<yade.utils._packPredicates.Predicate>` class. Predicate provides the following functionality:
...
CSG approach describes volume by geometric *primitives* or primitive solids (sphere, cylinder, box, cone, …) and boolean operations on them. Primitives defined in Yade include :ref:`inCylinder<yade._packPredicates.inCylinder>`, :ref:`inSphere<yade._packPredicates.inSphere>`, :ref:`inEllipsoid<yade._packPredicates.inEllipsoid>`, :ref:`inHyperboloid<yade._packPredicates.inHyperboloid>`, :ref:`notInNotch<yade._packPredicates.notInNotch>`.

For instance, :ref:`hyperboloid<yade._packPredicates.inHyperboloid>` (dogbone) specimen for tension-compression test can be constructed in this way (shown at img. img-hyperboloid_)::
...
#. :ref:`yade.pack.gtsSurface2Facets` function can create the triangulated surface (from :ref:`Facet` particles) in the simulation itself, as shown in the funnel example. (Triangulated surface can also be imported directly from a STL file using :ref:`yade.ymport.stl`.)
#. :ref:`yade._packPredicates.inGtsSurface` predicate can be created, using the surface as boundary representation of the enclosed volume.
...
.. figure:: fig/horse.png
	...
	Imported GTS surface (horse) used as packing predicate (top) and surface constructed from :ref:`facets<yade.utils.facet>` (bottom). See http://www.youtube.com/watch?v=PZVruIlUX1A for movie of this simulation.
...
* :ref:`intersection<yade._packPredicates.PredicateIntersection>` ``A & B`` (conjunction): point must be in both predicates involved.
* :ref:`union<yade._packPredicates.PredicateUnion>` ``A | B`` (disjunction): point must be in the first or in the second predicate.
* :ref:`difference<yade._packPredicates.PredicateDifference>` ``A - B`` (conjunction with second predicate negated): the point must be in the first predicate and not in the second one.
* :ref:`symmetric difference<yade._packPredicates.PredicateSymmetricDifference>` ``A ^ B`` (exclusive disjunction): point must be in exactly one of the two predicates.
...
Keyword arguments ``wire``, ``color`` and ``material`` are not declared in :ref:`yade.pack.randomDensePack`, therefore will be passed to :ref:`yade.utils.sphere`, where they are also documented. ``spheres`` is now list of :ref:`Body` objects, which we add to the simulation::

	O.bodies.append(spheres)

Packing algorithms described below produce dense packings. If one needs loose packing, :ref:`yade._packSpheres.SpherePack` class provides functions for generating loose packing, via its :ref:`yade._packSpheres.SpherePack.makeCloud` method. It is used internally for generating initial configuration in dynamic algorithms. For instance::
...
Regular 
"""""""""
Yade defines packing generators for spheres with constant radii, which can be used with volume predicates as described above. They are dense orthogonal packing (:ref:`yade.pack.regularOrtho`) and dense hexagonal packing (:ref:`yade.pack.regularHexa`). The latter creates so-called "hexagonal close packing", which achieves maximum density (http://en.wikipedia.org/wiki/Close-packing_of_spheres).
...
:ref:`yade._packSpherePadder.SpherePadder`
	constructs dense sphere packing based on pre-computed tetrahedron mesh; it is documented in :ref:`yade._packSpherePadder.SpherePadder` documentation; sample script is in :ysrc:`scripts/test/SpherePadder.py`. :ref:`yade._packSpherePadder.SpherePadder` does not return :ref:`Body` list as other algorithms, but a :ref:`yade._packSpheres.SpherePack` object; it can be iterated over, adding spheres to the simulation, as shown in its documentation.
GenGeo
	...
	Packings generated by this module can be imported directly via :ref:`yade.ymport.gengeo`, or from saved file via :ref:`yade.ymport.gengeoFile`. There is an example script :ysrc:`scripts/test/genCylLSM.py`. Full documentation for GenGeo can be found at `ESyS documentation website <http://esys.esscc.uq.edu.au/docs.html>`_.
...
The most versatile algorithm for random dense packing is provided by :ref:`yade.pack.randomDensePack`. Initial loose packing of non-overlapping spheres is generated by randomly placing them in cuboid volume, with radii given by requested (currently only uniform) radius distribution. When no more spheres can be inserted, the packing is compressed and then uncompressed (see :ysrc:`py/pack/pack.py` for exact values of these "stresses") by running a DEM simulation; :ref:`Omega.switchScene` is used to not affect existing simulation). Finally, resulting packing is clipped using provided predicate, as explained above.
...
* Providing ``memoizeDb`` parameter will make :ref:`yade.pack.randomDensePack` first look into provided file (SQLite database) for packings with similar parameters. On success, the packing is simply read from database and returned. If there is no similar pre-existent packing, normal procedure is run, and the result is saved in the database before being returned, so that subsequent calls with same parameters will return quickly.

If you need to obtain full periodic packing (rather than packing clipped by predicate), you can use :ref:`yade.pack.randomPeriPack`.

In case of specific needs, you can create packing yourself, "by hand". For instance, packing boundary can be constructed from :ref:`facets<Facet>`, letting randomly positioned spheres in space fall down under gravity.


Adding particles
================

The :ref:`BodyContainer` holds :ref:`Body` objects in the simulation; it is accessible as ``O.bodies``.

Creating Body objects
----------------------

:ref:`Body` objects are only rarely constructed by hand by their components (:ref:`Shape`, :ref:`Bound`, :ref:`State`, :ref:`Material`); instead, convenience functions :ref:`yade.utils.sphere`, :ref:`yade.utils.facet` and :ref:`yade.utils.wall` are used to create them. Using these functions also ensures better future compatibility, if internals of :ref:`Body` change in some way. These functions receive geometry of the particle and several other characteristics. See their documentation for details. If the same :ref:`Material` is used for several (or many) bodies, it can be shared by adding it in ``O.materials``, as explained below.

Defining materials
------------------

The ``O.materials`` object (instance of :ref:`Omega.materials`) holds defined shared materials for bodies. It only supports addition, and will typically hold only a few instance (though there is no limit).

``label`` given to each material is optional, but can be passed to :ref:`yade.utils.sphere` and other functions forconstructing body. The value returned by ``O.materials.append`` is an ``id`` of the material, which can be also passed to :ref:`yade.utils.sphere` -- it is a little bit faster than using label, though not noticeable for small number of particles and perhaps less convenient.

If no :ref:`Material` is specified when calling :ref:`yade.utils.sphere`, the *last* defined material is used; that is a convenient default. If no material is defined yet (hence there is no last material), a default material will be created using :ref:`yade.utils.defaultMaterial`; this should not happen for serious simulations, but is handy in simple scripts, where exact material properties are more or less irrelevant.
...
* packing generators, such as :ref:`yade.pack.randomDensePack`, :ref:`yade.pack.regularHexa`
* surface function :ref:`yade.pack.gtsSurface2Facets`
* import functions :ref:`yade.ymport.gmsh`, :ref:`yade.ymport.stl`, …

As those functions use :ref:`yade.utils.sphere` and :ref:`yade.utils.facet` internally, they accept additional argument passed to those function. In particular, material for each body is selected following the rules above (last one if not specified, by label, by index, etc.).


Clumping particles together
----------------------------

In some cases, you might want to create rigid aggregate of individual particles (i.e. particles will retain their mutual position during simulation); a special function :ref:`BodyContainer.appendClumped` is designed for this task; for instance, we might add 2 spheres tied together:
...
:ref:`appendClumped<BodyContainer.appendClumped>` returns a tuple of ``(clumpId,[memberId1,memberId2])``: clump is internally represented by a special :ref:`Body`, referenced by :ref:`clumpId<Body.clumpId>` of its members (see also  :ref:`isClump<Body.isClump>`, :ref:`isClumpMember<Body.isClumpMember>` and :ref:`isStandalone<Body.isStandalone>`).
...
In typical cases, interactions are created during simulations as particles collide. This is done by a :ref:`Collider` detecting approximate contact between particles and then an :ref:`IGeomFunctor` detecting exact collision.

Some material models (such as the :ref:`concrete model<Law2_Dem3DofGeom_CpmPhys_Cpm>`) rely on initial interaction network which is denser than geometrical contact of spheres: sphere's radii as "enlarged" by a dimensionless factor called *interaction radius* (or *interaction ratio*) to create this initial network. This is done typically in this way (see :ysrc:`examples/concrete/uniax.py` for an example):

#. Approximate collision detection is adjusted so that approximate contacts are detected also between particles within the interaction radius. This consists in setting value of :ref:`Bo1_Sphere_Aabb.aabbEnlargeFactor` to the interaction radius value.

#. The geometry functor (``Ig2``)
   would normally say that "there is no contact" if given 2 spheres that are not in contact. Therefore, the same value as for :ref:`Bo1_Sphere_Aabb.aabbEnlargeFactor` must be given to it. (Either :ref:`Ig2_Sphere_Sphere_Dem3DofGeom.distFactor` or :ref:`Ig2_Sphere_Sphere_ScGeom.interactionDetectionFactor`, depending on the functor that is in use.

   Note that only :ref:`Sphere` + :ref:`Sphere` interactions are supported; there is no parameter analogous to :ref:`distFactor<Ig2_Sphere_Sphere_Dem3DofGeom.distFactor>` in :ref:`Ig2_Facet_Sphere_Dem3DofGeom`. This is on purpose, since the interaction radius is meaningful in bulk material represented by sphere packing, whereas facets usually represent boundary conditions which should be exempt from this dense interaction network.
...
This method will be rather slow if many interaction are to be created (the functor lookup will be repeated for each of them). In such case, ask on yade-dev@lists.launchpad.net to have the :ref:`yade.utils.createInteraction` function accept list of pairs id's as well.
...
The order of engines is important. In majority of cases, you will put any additional engine after :ref:`InteractionLoop`:

* if it apply force, it should come before :ref:`NewtonIntegrator`, otherwise the force will never be effective.
* if it makes use of bodies' positions, it should also come before :ref:`NewtonIntegrator`, otherwise, positions at the next step will be used (this might not be critical in many cases, such as output for visualization with :ref:`VTKRecorder`).

The :ref:`O.engines<Omega.engines>` sequence must be always assigned at once (the reason is in the fact that although engines themselves are passed by reference, the sequence is *copied* from c++ to Python or from Python to c++). This includes modifying an existing ``O.engines``; therefore ::
...
Bo1 functors
^^^^^^^^^^^^
``Bo1`` functors must be chosen depending on the collider in use; they are given directly to the collider (which internally uses :ref:`BoundDispatcher`).

At this moment (September 2010), the most common choice is :ref:`InsertionSortCollider`, which uses :ref:`Aabb`; functors creating :ref:`Aabb` must be used in that case. Depending on particle :ref:`shapes<Shape>` in your simulation, choose appropriate functors::
...
Using more functors than necessary (such as :ref:`Bo1_Facet_Aabb` if there are no :ref:`facets<Facet>` in the simulation) has no performance penalty. On the other hand, missing functors for existing :ref:`shapes<Shape>` will cause those bodies to not collider with other bodies (they will freely interpenetrate).

There are other colliders as well, though their usage is only experimental:

* :ref:`SpatialQuickSortCollider` is correctness-reference collider operating on :ref:`Aabb`; it is significantly slower than :ref:`InsertionSortCollider`.
* :ref:`PersistentTriangulationCollider` only works on spheres; it does not use a :ref:`BoundDispatcher`, as it operates on spheres directly.
* :ref:`FlatGridCollider` is proof-of-concept grid-based collider, which computes grid positions internally (no :ref:`BoundDispatcher` either)

Ig2 functors
^^^^^^^^^^^^^

``Ig2`` functor choice (all of the derive from :ref:`IGeomFunctor`) depends on 
...
#. shape combinations that should collide;
   ...
   will handle collisions for :ref:`Sphere` + :ref:`Sphere`, but not for :ref:`Facet` + :ref:`Sphere` -- if that is desired, an additional functor must be used::
...
#. :ref:`IGeom` type accepted by the ``Law2`` functor (below); it is the first part of functor's name after ``Law2`` (for instance, :ref:`Law2_Dem3DofGeom_CpmPhys_Cpm` accepts :ref:`Dem3DofGeom`). This is (for most cases) either :ref:`Dem3DofGeom` (total shear formulation) or :ref:`ScGeom` (incremental shear formulation). For :ref:`ScGeom`, the above example would simply change to::
...
``Ip2`` functors (deriving from :ref:`IPhysFunctor`) must be chosen depending on

#. :ref:`Material` combinations within the simulation. In most cases, ``Ip2`` functors handle 2 instances of the same :ref:`Material` class (such as :ref:`Ip2_FrictMat_FrictMat_FrictPhys` for 2 bodies with :ref:`FrictMat`) 

#. :ref:`IPhys` accepted by the constitutive law (``Law2`` functor), which is the second part of the ``Law2`` functor's name (e.g. :ref:`Law2_ScGeom_FrictPhys_CundallStrack` accepts :ref:`FrictPhys`)

.. note:: Unlike with ``Bo1`` and ``Ig2`` functors, unhandled combination of :ref:`Materials<Material>` is an error condition signaled by an exception.
...
In most simulations, only one ``Law2`` functor will be in use; it is possible, though, to have several of them, dispatched based on combination of :ref:`IGeom` and :ref:`IPhys` produced previously by ``Ig2`` and ``Ip2`` functors respectively (in turn based on combination of :ref:`Shapes<Shape>` and :ref:`Materials<Material>`).

.. note:: As in the case of ``Ip2`` functors, receiving a combination of :ref:`IGeom` and :ref:`IPhys` which is not handled by any ``Law2`` functor is an error.
...
Basic DEM model
^^^^^^^^^^^^^^^^
Suppose we want to use the :ref:`Law2_ScGeom_FrictPhys_CundallStrack` constitutive law. We see that

#. the ``Ig2`` functors must create :ref:`ScGeom`. If we have for instance :ref:`spheres<Sphere>` and :ref:`boxes<Box>` in the simulation, we will need functors accepting :ref:`Sphere` + :ref:`Sphere` and :ref:`Box` + :ref:`Sphere` combinations. We don't want interactions between boxes themselves (as a matter of fact, there is no such functor anyway). That gives us :ref:`Ig2_Sphere_Sphere_ScGeom` and :ref:`Ig2_Box_Sphere_ScGeom`.

#. the ``Ip2`` functors should create :ref:`FrictPhys`. Looking at :ref:`InteractionPhysicsFunctors<IPhysFunctor>`, there is only :ref:`Ip2_FrictMat_FrictMat_FrictPhys`. That obliges us to use :ref:`FrictMat` for particles.
...
Concrete model
^^^^^^^^^^^^^^^
In this case, our goal is to use the :ref:`Law2_Dem3DofGeom_CpmPhys_Cpm` constitutive law.

* We use :ref:`spheres<Sphere>` and :ref:`facets<Facet>` in the simulation, which selects ``Ig2`` functors accepting those types and producing :ref:`Dem3DofGeom`: :ref:`Ig2_Sphere_Sphere_Dem3DofGeom` and :ref:`Ig2_Facet_Sphere_Dem3DofGeom`.

* We have to use :ref:`Material` which can be used for creating :ref:`CpmPhys`. We find that :ref:`CpmPhys` is only  created by :ref:`Ip2_CpmMat_CpmMat_CpmPhys`, which determines the choice of :ref:`CpmMat` for all particles.
...
* :ref:`Body.dynamic` determines whether a body will be moved by :ref:`NewtonIntegrator`; it is mandatory for bodies with zero mass, where applying non-zero force would result in infinite displacement.

  :ref:`Facets<Facet>` are case in the point: :ref:`yade.utils.facet` makes them non-dynamic by default, as they have zero volume and zero mass (this can be changed, by passing ``dynamic=True`` to :ref:`yade.utils.facet` or setting :ref:`Body.dynamic`; setting :ref:`State.mass` to a non-zero value must be done as well). The same is true for :ref:`yade.utils.wall`.
...
* :ref:`State.blockedDOFs` permits selective blocking
  ...
  In contrast to :ref:`Body.dynamic`, :ref:`blockedDOFs<State.blockedDOFs>` will only block forces (and acceleration) in that direction being effective; if you prescribed linear or angular velocity, they will be applied regardless of :ref:`blockedDOFs<State.blockedDOFs>`. (This is also related to `bug #398089 <https://bugs.launchpad.net/yade/+bug/398089>`_ mentioned above)
...
Engines deriving from :ref:`BoundaryController` impose boundary conditions during simulation, either directly, or by influencing several bodies. You are referred to their individual documentation for details, though you might find interesting in particular

* :ref:`UniaxialStrainer` for applying strain along one axis at constant rate; useful for plotting strain-stress diagrams for uniaxial loading case. See :ysrc:`examples/concrete/uniax.py` for an example.
* :ref:`TriaxialStressController` which applies prescribed stress/strain along 3 perpendicular axes on cuboid-shaped packing using 6 walls (:ref:`Box` objects) (:ref:`ThreeDTriaxialEngine` is generalized such that it allows independent value of stress along each axis)
* :ref:`PeriTriaxController` for applying stress/strain along 3 axes independently, for simulations using periodic boundary conditions (:ref:`Cell`)

Field appliers
---------------

Engines deriving from :ref:`FieldApplier` acting on all particles. The one most used is :ref:`GravityEngine` applying uniform acceleration field.

Partial engines
---------------

Engines deriving from :ref:`PartialEngine` define the :ref:`subscribedBodies<PartialEngine.subscribedBodies>` attribute determining bodies which will be affected. Several of them warrant explicit mention here:

* :ref:`TranslationEngine` and :ref:`RotationEngine` for applying constant speed linear and rotational motion on subscribers. 
* :ref:`ForceEngine` and :ref:`TorqueEngine` applying given values of force/torque on subscribed bodies at every step.
* :ref:`StepDisplacer` for applying generalized displacement delta at every timestep; designed for precise control of motion when testing constitutive laws on 2 particles.

If you need an engine applying non-constant value instead, there are several interpolating engines (:ref:`InterpolatingDirectedForceEngine` for applying force with varying magnitude, :ref:`InterpolatingSpiralEngine` for applying spiral displacement with varying angular velocity and possibly others); writing a new interpolating engine is rather simple using examples of those that already exist.
...
:ref:`Omega.tags` is a dictionary (it behaves like a dictionary, although the implementation in c++ is different) mapping keys to labels. Contrary to regular python dictionaries that you could create,
...
Python variable lifetime is limited; in particular, if you save simulation, variables will be lost after reloading. Yade provides limited support for data persistence for this reason (internally, it uses special values of ``O.tags``). The functions in question are :ref:`yade.utils.saveVars` and :ref:`yade.utils.loadVars`. 

:ref:`yade.utils.saveVars` takes dictionary (variable names and their values) and a *mark* (identification string for the variable set); it saves the dictionary inside the simulation. These variables can be re-created (after the simulation was loaded from a XML file, for instance) in the ``yade.params.``\ *mark* namespace by calling :ref:`yade.utils.loadVars` with the same identification *mark*:
...
.. note:: Only types that can be `pickled <http://docs.python.org/library/pickle.html>`_ can be passed to :ref:`yade.utils.saveVars`.
...
A special engine :ref:`PyRunner` can be used to periodically call python code, specified via the ``command`` parameter. Periodicity can be controlled by specifying computation time (``realPeriod``), virutal time (``virtPeriod``) or iteration number (``iterPeriod``).

For instance, to print kinetic energy (using :ref:`yade.utils.kineticEnergy`) every 5 seconds, this engine will be put to ``O.engines``::
...
The :ref:`yade.plot` module provides simple interface and storage for tracking various data. Although originally conceived for plotting only, it is widely used for tracking variables in general.

The data are in :ref:`yade.plot.data` dictionary, which maps variable names to list of their values; the :ref:`yade.plot.addData` function is used to add them.
...
New record is added to all columns at every time :ref:`yade.plot.addData` is called; this assures that lines in different columns always match. The special value ``nan`` or ``NaN`` (`Not a Number <http://en.wikipedia.org/wiki/NaN>`_) is inserted to mark the record invalid.

.. note:: It is not possible to have two columns with the same name, since data are stored as a dictionary.

To record data periodically, use :ref:`PyRunner`. This will record the *z* coordinate and velocity of body #1, iteration number and simulation time (every 20 iterations)::
...
.. note::
	...
	Arbitrary string can be used as column label for :ref:`yade.plot.data`. If it cannot be used as keyword name for :ref:`yade.plot.addData` (since it is a python keyword (``for``), or has spaces inside (``my funny column``), you can pass dictionary to :ref:`yade.plot.addData` instead::

		plot.addData(z=b.state.pos[2],**{'my funny column':b.state.vel.norm()})

	An exception are columns having leading of trailing whitespaces. They are handled specially in :ref:`yade.plot.plots` and should not be used (see below).
...
In that case, naturally, the labeled object must define attributes which are used (:ref:`UniaxialStrainer.strain` and :ref:`UniaxialStrainer.stress` in this case).

Plotting variables
-------------------
Above, we explained how to track variables by storing them using :ref:`yade.plot.addData`. These data can be readily used for plotting. Yade provides a simple, quick to use, plotting in the :ref:`yade.plot` module. Naturally, since direct access to underlying data is possible via :ref:`yade.plot.data`, these data can be processed in any way.

The :ref:`yade.plot.plots` dictionary is a simple specification of plots. Keys are x-axis variable, and values are tuple of y-axis variables, given as strings that were used for :ref:`yade.plot.addData`; each entry in the dictionary represents a separate figure::
...
Actual plot using data in :ref:`yade.plot.data` and plot specification of :ref:`yade.plot.plots` can be triggered by invoking the :ref:`yade.plot.plot` function.
...
* :ref:`yade.plot.live` - By setting ``yade.plot.live=True`` you can watch the plot being updated while the calculations run. Set to ``False`` otherwise.
* :ref:`yade.plot.liveInterval` - This is the interval in seconds between the plot updates.
* :ref:`yade.plot.autozoom` - When set to ``True`` the plot will be automatically rezoomed.
...
Since :ref:`yade.plot.plots` is a dictionary, multiple entries with the same key (x-axis variable) would not be possible, since they overwrite each other:
...
You can, however, distinguish them by prepending/appending space to the x-axis variable, which will be removed automatically when looking for the variable in :ref:`yade.plot.data` -- both $x$-axes will use the ``i`` column:
...
Plots can be exported to external files for later post-processing via that :ref:`yade.plot.saveGnuplot` function.
...
* The gnuplot file can be run through gnuplot to produce the figure; see :ref:`yade.plot.saveGnuplot` documentation for details.
...
Frequently, decisions have to be made based on evolution of the simulation itself, which is not yet known. In such case, a function checking some specific condition is called periodically; if the condition is satisfied, ``O.pause`` or other functions can be called to stop the stimulation. See documentation for :ref:`Omega.run`, :ref:`Omega.pause`, :ref:`Omega.step`, :ref:`Omega.stopAtIter` for details.

For simulations that seek static equilibrium, the :ref:`yade._utils.unbalancedForce` can provide a useful metrics (see its documentation for details); for a desired value of ``1e-2`` or less, for instance, we can use::
...
Arbitrary functions can be periodically checked, and they can also use history of variables tracked via :ref:`yade.plot.addData`. For example, this is a simplified version of damage control in :ysrc:`examples/concrete/uniax.py`; it stops when current stress is lower than half of the peak stress::
...
simulation script
	regular Yade script, which calls :ref:`yade.utils.readParamsFromTable` to obtain parameters from parameter table. In order to make the script runnable outside the batch, :ref:`yade.utils.readParamsFromTable` takes default values of parameters, which might be overridden from the parameter table.
	
	:ref:`yade.utils.readParamsFromTable` knows which parameter file and which line to read by inspecting the ``PARAM_TABLE`` environment variable, set by the batch system.

parameter table
	simple text file, each line representing one parameter set. This file is read by :ref:`yade.utils.readParamsFromTable` (using :ref:`yade.utils.TableParamReader` class), called from simulation script, as explained above.
...
after the call to :ref:`yade.utils.readParamsFromTable`, corresponding python variables are created in the ``yade.params.table`` module and can be readily used in the script, e.g.
...
----------------------------------
As one might output data to external files during simulation (using classes such as :ref:`VTKRecorder`, it is important to name files in such way that they are not overwritten by next (or concurrent) job in the same batch. A special tag ``O.tags['id']`` is provided for such purposes: it is comprised of date, time and PID, which makes it always unique (e.g. ``20100413T144723p7625``); additional advantage is that alphabetical order of the ``id`` tag is also chronological.
...
#. Periodic frame snapshot using :ref:`SnapshotEngine` (see :ysrc:`examples/bulldozer.py` for a full example)::
   ...
   which will save numbered files like ``/tmp/bulldozer-0000.png``. These files can be processed externally (with `mencoder <http://www.mplayerhq.hu>`_ and similar tools) or directly with the :ref:`yade.utils.encodeVideoFromFrames`::
...
Paraview is based on the `Visualization Toolkit <http://www.vtk.org>`_, which defines formats for saving various types of data. One of them (with the ``.vtu`` extension) can be written by a special engine :ref:`VTKRecorder`. It is added to the simulation loop::
...
* :ref:`iterPeriod<PeriodicEngine.iterPeriod>` determines how often to save simulation data (besides :ref:`iterPeriod<PeriodicEngine.iterPeriod>`, you can also use :ref:`virtPeriod<PeriodicEngine.virtPeriod>` or :ref:`realPeriod<PeriodicEngine.realPeriod>`). If the period is too high (and data are saved only few times), the video will have few frames. 
* :ref:`fileName<VTKRecorder.fileName>` is the prefix for files being saved. In this case, output files will be named ``/tmp/p1-spheres.0.vtu`` and ``/tmp/p1-facets.0.vtu``, where the number is the number of iteration; many files are created, putting them in a separate directory is advisable.
* :ref:`recorders<VTKRecorder.recorders>` determines what data to save (see the :ref:`documentation<VTKRecorder.recorders>`)
